...
class App:
   ...
   def currancy_exchange(self, e):
      ...
      # Currency Entry Input Frame
      self.currency_entry_frame = customtkinter.CTkFrame(self.root)
      self.currency_entry_frame.pack(side=TOP, anchor="w", padx=(15, 0), pady=(7, 0))

      # Currency Entry Input (First Exchanger) ######
      self.first_currency_input = customtkinter.CTkEntry(self.currency_entry_frame, text_font=("Arial", 26), width=70, height=44)
      self.first_currency_input.grid(row=0, column=0)
      # ComboBox 1
      self.first_combobox_var = customtkinter.StringVar(value="USD")  # set initial value
      self.combobox1 = customtkinter.CTkComboBox(self.currency_entry_frame, values=exchanges, variable=self.first_combobox_var, width=70, height=30)
      self.combobox1.grid(row=1, column=0)

      # Exchange Label Between The Two Input Entries
      self.exchange_label = customtkinter.CTkLabel(self.currency_entry_frame, text="Exchange", width=0, text_font=("Arial", 14))
      self.exchange_label.grid(row=0, column=1, padx=5)


      # Currency Entry Input (Second Exchanger) ######
      self.second_currency_input = customtkinter.CTkEntry(self.currency_entry_frame, text_font=("Arial", 26), width=70, height=44)
      self.second_currency_input.grid(row=0, column=2)
      # ComboBox 2
      self.second_combobox_var = customtkinter.StringVar(value="USD")  # set initial value
      self.combobox2 = customtkinter.CTkComboBox(self.currency_entry_frame, values=exchanges, variable=self.second_combobox_var, width=70, height=30)
      self.combobox2.grid(row=1, column=2)

      # Exchange Button ~ Frame
      self.exchange_btn_frame = customtkinter.CTkFrame(self.root)
      self.exchange_btn_frame.pack(pady=(7, 0))

      # Exchange Button ~ Button
      self.exchange_btn = customtkinter.CTkButton(self.exchange_btn_frame, text="Button", text_font=("Arial", 16), width=200,
                  command=lambda: self.exchange_rate(self.second_currency_input.get(), self.first_currency_input.get(), self.combobox1.get(), self.combobox2.get()))
      self.exchange_btn.pack()
   ...
   def return_standard(self, e):
      self.currency_entry_frame.pack_forget()
      self.currency_label_frame.pack_forget()
      self.exchange_btn_frame.pack_forget()
...
# TODO fix up all the number btn indents
# TODO create error for when user types in str instead of int
# TODO add number options to currency exchange
# TODO add more buttons to advanced option
# TODO had output labels for currency exchange option
# TODO look up currency exchange APIs
